...
owner = ["361534796830081024", "365173881952272384"]
...
async def on_ready():
    ...
    await bot.change_presence(game=discord.Game(name='Coding in ' + len(bot.servers)))
...
async def setname(ctx, *, name):
    ...
    if name != "":
        try:
            ...
            await bot.send_message(message.channel, "**Failed to change name**")
        else:
            await bot.send_message(message.channel, "**Successfuly changed name to {}**".format(name))
    else:
        await bot.send_cmd_help(ctx)

@bot.command(pass_contex=True, hidden=True)
async def setgame(ctx, *, game):
    game = game.strip()
    if game != "":
        try:
            await bot.presence(game=discord.Game(name=game))
        except:
